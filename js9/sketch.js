...
let finishButton; // Кнопка для завершения рисунка
...
function setup() {
    ...
    // Кнопка "Закончил рисунок" в верхнем левом углу
    finishButton = createButton('Закончил рисунок');
    finishButton.position(10, 10); // Размещение кнопки
    finishButton.mousePressed(toggleFinishButton);
}
...
function toggleFinishButton() {
    if (editMode === 0) {
        finishButton.html('Молодец');  // Если рисование завершено, меняем на "Молодец"
        finishButton.style('background-color', 'green'); // Зеленый цвет
    } else {
        finishButton.html('Закончил рисунок');  // Если рисование не завершено, возвращаем текст
        finishButton.style('background-color', 'red'); // Красный цвет
    }
}